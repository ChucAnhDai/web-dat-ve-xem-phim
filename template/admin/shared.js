 ...
 * ─────────────────────────────────────────────
 * 1. loadComponents()   — fetch sidebar/header/footer HTML and inject into page
 * 2. initComponents()   — run after components are loaded (active nav, search, etc.)
 * 3. Helper functions   — toast, modal, sidebar toggle, status badges, pagination
 *
 * HOW PAGES USE THIS:
 *   • Place <div id="sidebarMount"></div>, <div id="headerMount"></div>,
 *     <div id="footerMount"></div> in the HTML
 *   • Set window.QUICK_ACTION = { label:'Add Movie', fn: () => openModal(...) }
 *     BEFORE the DOMContentLoaded event if you want a custom quick-add button
 *   • Listen for 'componentsReady' event for page-specific init that depends
 *     on components being in the DOM
 */
...
// ══════════════════════════════════════════════
// COMPONENT LOADER  (fetch → inject)
// ══════════════════════════════════════════════
function _loadHTML(url, targetId) {
  const target = document.getElementById(targetId);
  if (!target) return Promise.resolve();          // mount point not on this page
  return fetch(url)
    .then(r => { if (!r.ok) throw new Error(url + ' ' + r.status); return r.text(); })
    .then(html => { target.innerHTML = html; })
    .catch(err => {
      console.warn('[CineShop] component load failed:', err.message);
      // fallback: hide the mount point silently
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load the three shared components in parallel
  Promise.all([
    _loadHTML('sidebar.html', 'sidebarMount'),
    _loadHTML('header.html',  'headerMount'),
    _loadHTML('footer.html',  'footerMount'),
  ]).then(() => {
